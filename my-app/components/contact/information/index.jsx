...
const index = () => {
  ...
  const {
    datas: staticTextsData,
    loading: staticTextsLoading,
    error: staticTextsError,
  } = useProductData("/static-texts");
  ...
  async function getAlertMessage() {
    ...
    try {
      const response = await fetch(
        "https://admin.almetbaku.az/api/static-texts",
        {
          headers: {
    ...
    <div className="grid grid-cols-12 gap-4 lg:my-20 my-10">
      <div className="lg:col-span-4 sm:col-span-6 col-span-12">
        ...
        <div className="my-4 flex sm:justify-start justify-center">
          ...
          >
            <Link href="https://linktr.ee/almetbaku" target="_blank">
              {" "}
              <h5 className={styles.keepInTouch}>
                {staticTextsData.data.contact_keep_in}
              </h5>{" "}
            </Link>
            <Image
        ...
        <div className="mt-10">
          <h4 className={`${styles.turnIdea} ${roboto2.className}`}>
            {staticTextsData.data.contact_we_are_ready}
            <br /> {staticTextsData.data.contact_your_idea}
          </h4>
        </div>
      </div>
      <div className="lg:col-span-8 sm:col-span-6 col-span-12">
        <div>
          <div className={styles.message}>
            <h4 className={roboto3.className}>
              {staticTextsData.data.contact_ready_to_start}
            </h4>
            <h4 className={roboto3.className}>
              {staticTextsData.data.contact_believe_in_our}
            </h4>
            <h4 className={roboto3.className}>
              {staticTextsData.data.contact_share_your}
            </h4>
          </div>
        </div>
        ...
        <div className="mt-8 flex justify-end">
          ...
          >
            {staticTextsData.data.contact_message_btn}
          </button>
        </div>

        {success && (
          <div className={`${styles.success} ${styles.successFinish}`}>
      ...
      <div className="col-span-12 flex flex-col items-center my-16">
        <div className="my-16" style={{ height: "270px", width: "100%" }}>
          <YMaps>
            <div>
              <Map
                defaultState={{
                  center: [40.4283289, 49.8685741],
                  zoom: 12,
                }}
                width="100%"
                height={350}
              >
                <Placemark geometry={[40.4283289, 49.8685741]} />
              </Map>
            </div>
          </YMaps>
        </div>
